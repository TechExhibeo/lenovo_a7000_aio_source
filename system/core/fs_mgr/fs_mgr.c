    ...
    for (i = 0; i < fstab->num_entries; i++) {
        ...
        if (!mret) {
            ...
            if ((fstab->recs[attempted_idx].fs_mgr_flags & MF_FORCECRYPT)) {
                ...
                //Lenovo-sw,Start, EasyImage, miaotao1
                //If we have easyimage.zip in data, do not force ecrypt as it's still in factory process
                FILE * easyImageFile = fopen("/data/easyimage.zip","r");
                if(easyImageFile!=NULL)
                {
				  fclose(easyImageFile);
                  continue;
                }
                //Lenovo-sw, End,EasyImage,miaotao1
