...
static void derive_permissions_locked(struct fuse* fuse, struct node *parent,
    ...
    case PERM_ROOT:
        ...
        // { huangzq2, Secure Zone: Access open user data via OpenUserData folder
        } else if (node->userid == 10 && !strcasecmp(node->name, "OpenUserData")) {
            node->graft_path = "/data/media/0";
            node->graft_pathlen = strlen("/data/media/0");
        }
        // huangzq2, Secure Zone }
        break;
    case PERM_ANDROID:
...
static int run(const char* source_path, const char* dest_path, uid_t uid,
    ...
    res = mount("/dev/lefuse", dest_path, "lefuse", MS_NOSUID | MS_NODEV, opts);
    if (res < 0) {
